...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  // Check if the topic is "firmware"
  if (String(topic) == "intimate") {
    DynamicJsonDocument doc(256);
    deserializeJson(doc, payload, length);
    float available_version = doc["version"];
    ...
    float curr_version = preferences.getFloat("version", 1.0);  // Default to 1.0 if not set
    preferences.end();
    ...
    if (available_version > curr_version) {
      ...
      if (downloadAndUpdateFirmware("http://192.168.123.250:4000/request")) {
        preferences.begin("firmware", false);
        preferences.putFloat("version", available_version);
        preferences.end();
